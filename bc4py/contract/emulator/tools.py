...
def broadcast(c_address, start_tx, redeem_address, emulate_gas, result, f_not_send=False):
    ...
    if isinstance(result, tuple) and len(result) == 2:
        ...
        return None
    # create conclude tx
    conclude_tx = create_conclude_tx(
        c_address=c_address,
        start_tx=start_tx,
        redeem_address=redeem_address,
        send_pairs=send_pairs,
        c_storage=c_storage,
        emulate_gas=emulate_gas)
    # send tx
    ...
    if another_conclude_hash is not None:
        ...
        a_another = calc_tx_movement(
            tx=another_tx, c_address=c_address, redeem_address=redeem_address, emulate_gas=emulate_gas)
        a_another.cleanup()
        _c_address, _start_hash, another_storage = another_tx.encoded_message()
        log.error("Failed confirm ConcludeTX, please check params\n"
                  "   AnoAccount=>{}\n   MyAccount =>{}\n"
                  "   AnoStorage=>{}\n   MyStorage =>{}\n"
                  "   AnoTX=>{}\n   MyTX =>{}\n".format(a_another, a_conclude, another_storage, c_storage,
                                                        another_tx, conclude_tx))
        return None
