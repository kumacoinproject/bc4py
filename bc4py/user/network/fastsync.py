...
async def main_sync_loop():
    while not P.F_STOP:
        ...
        if P.F_NOW_BOOTING is False:
            continue
        if obj.chain_builder.best_block is None:
            continue
        # start fast sync
        my_best_block: Block = obj.chain_builder.best_block
        ...
        while True:
            ...
            if new_block is None:
                ...
                stack_dict.clear()
                break
            if obj.chain_builder.root_block is not None\
                    and obj.chain_builder.root_block.height is not None\
                    and new_block.height <= obj.chain_builder.root_block.height:
                log.error("cannot rollback block depth height={}".format(new_block.height))
                P.F_STOP = True
                return
            if new_block.hash in obj.chain_builder.chain:
                log.debug("new block is already known {}".format(new_block))
                my_best_block = obj.chain_builder.get_block(blockhash=new_block.hash)
                continue
            if my_best_block.hash != new_block.previous_hash:
                ...
                my_best_block = obj.chain_builder.get_block(blockhash=my_best_block.previous_hash)
                await back_que.put(my_best_block.height + 1)
                continue
